    ...
    return (
        <div className={'english__header'}>
            <div className={'english__header__button__container'}>
                <button className={prop.state.settings.soundButton ? 'button__active' : 'header__button__english'}
                        onClick={() => showSound()}>Слушать пол
                </button>
                <button className={prop.state.settings.translation ? 'button__active' : 'header__button__english'}
                        onClick={() => showTranslation()}>Перевод
                </button>
                <button className={prop.state.settings.autoSound ? 'button__active' : 'header__button__english'}
                        onClick={() => autoSound()}>Слушать
                </button>
                <button className={prop.state.settings.picture ? 'button__active' : 'header__button__english'}
                        onClick={() => showBg()}>показать картинку
                </button>
            </div>
        </div>
    )
