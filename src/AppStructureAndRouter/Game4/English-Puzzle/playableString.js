...
export default function PlayableString(prop) {
    ...
    function addWord() {
        ...
        if(chosenWords.indexOf('') === -1 && chosenWords.length !== 0){ check = true}
    ...
    let styling = '';
    if(prop.word !== '' && gameContext.gameState.settings.picture === true){styling = style.div}else{styling = style.styleNone}
    return (
